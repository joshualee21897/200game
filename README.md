...
- **Call resolution — ties and wrong calls.** A caller who is strictly lowest wins (adds 0, everyone else adds their own value). If someone *ties* the caller's value (no one strictly lower), it's a push, like tying the dealer: neither the caller nor the tying player(s) add anything, but anyone else at the table still adds their own value. If anyone is *strictly* lower, it's a wrong call: the caller adds a flat 30-point penalty only (not their hand value on top of it), and only whoever has the single lowest hand among the non-callers adds nothing — everyone else, including anyone who merely beat the caller without being the overall lowest, still adds their own value.
